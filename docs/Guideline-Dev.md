...
⬜ 2.2  Migration สำหรับ 3 ตารางใหม่
...
⬜ 2.8  TransactionController: GET (list + pagination + filter)
...
⬜ 2.19 Transaction list page (table + pagination)
⬜ 2.20 Filter: by date range + category + type
⬜ 2.21 Add Transaction form (Reactive Forms)
...
**Testing:**
```
⬜ 2.25 Unit tests: TransactionService (5+ tests, xUnit + Moq)
⬜ 2.26 Integration tests: TransactionController (2+ tests)
```
...
⬜ 3.2  สร้าง ClaudeAiService (implementation)
⬜ 3.3  Prompt template: categorize transaction
⬜ 3.4  Prompt template: generate insights
...
**CSV Import:**
```
⬜ 3.10 CSV upload endpoint
⬜ 3.11 CSV parser (handle different formats, encoding)
⬜ 3.12 Batch categorization (ส่งหลาย transactions ไป AI ทีเดียว)
⬜ 3.13 Background job (Hangfire) สำหรับ batch ใหญ่
⬜ 3.14 Progress tracking (user เห็น % ที่ process แล้ว)
⬜ 3.15 Error handling: partial failure (บาง row ผ่าน บาง row ไม่ผ่าน)
```

**AI Insights:**
```
⬜ 3.16 Insights endpoint: รับ transactions 30 วัน → ส่ง Claude → return insights
⬜ 3.17 Insight types: overspending, trend, anomaly, saving opportunity
⬜ 3.18 Cache insights (invalidate เมื่อมี transaction ใหม่)
```

**Frontend:**
```
⬜ 3.19 CSV Import page (upload + preview + progress + result)
⬜ 3.20 AI category suggestion ใน Add Transaction form
⬜ 3.21 Category review page (หลัง import เสร็จ)
⬜ 3.22 Insights card component (แสดงบน dashboard)
⬜ 3.23 Loading states สำหรับ AI calls (อาจนาน 3-10 วินาที)
```
