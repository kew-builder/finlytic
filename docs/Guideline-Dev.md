...
| Phase 4: Dashboard | ✅ | 2026-05-17 | 2026-05-18 |
| Phase 5: Polish & Deploy | 🔄 | 2026-05-20 | - |
...
### Definition of Done
- [x] `docs/requirements.md` อยู่ใน repo
- [x] `docs/architecture/system-diagram.png` อยู่ใน repo
- [x] `docs/flows/` มี flow 2 ไฟล์
- [x] `docs/database/schema.dbml` อยู่ใน repo
- [x] `docs/decisions/ADR-001.md` อยู่ใน repo
- [x] GitHub repo มี branch protection on main
- [x] Claude Design prototype ของทุกหน้าเสร็จ
...
### Definition of Done
- [x] สามารถ register user ใหม่ได้
- [x] สามารถ login ได้ → redirect ไป dashboard
- [x] Dashboard แสดง "Welcome, [name]"
- [x] Refresh token ทำงาน (token หมดอายุ → auto-refresh)
- [x] ถ้าไม่ได้ login → redirect ไป login page
- [x] Swagger UI เปิดได้ที่ /swagger
...
### Definition of Done
- [x] CRUD ทำงานครบ (Create, Read, Update, Delete)
- [x] Pagination + Filter ทำงาน
- [x] Validation ทำงานทั้ง frontend + backend
- [x] มี unit tests + integration tests
- [x] ADR-002 เขียนเสร็จ
- [x] เข้าใจ Include vs AsNoTracking vs Select ลึกพอตอบ interview ได้
...
### Definition of Done
- [x] Import CSV → AI categorize → save → user review ทำงาน end-to-end
- [x] Add Transaction → AI suggest category ทำงาน
- [x] Dashboard แสดง AI insights
- [x] ถ้า AI ล่ม → app ยังทำงานได้ (graceful degradation)
- [x] มี token usage log
- [x] ADR-003 เขียนเสร็จ
...
✅ 4.4  Cash flow forecast API (Gemini AI predict)
✅ 4.5  Budget CRUD API
✅ 4.6  Performance: add indexes (AddDashboardIndexes migration — UserId+TransactionDate composite index)
```
...
✅ 4.9  Donut chart: spending by category (Chart.js)
✅ 4.10 Line chart: Monthly Cash Flow — Income vs Expenses 6 months
✅ 4.11 Bar chart: budget vs actual
✅ 4.12 Forecast chart: predicted next 3 months (line chart)
✅ 4.13 Budget management page (set budget per category)
✅ 4.14 Period toggle: เดือนนี้ / 3 เดือน / 6 เดือน
✅ 4.15 Responsive design (responsive grid layout)
```

### Definition of Done
- [x] Dashboard แสดง charts ครบ 4 อัน
- [x] Charts update เมื่อเปลี่ยน date range
- [x] Budget management ทำงาน
- [x] Forecast จาก AI ทำงาน
- [x] หน้า dashboard โหลด < 2 วินาที
- [x] Mobile responsive
...
✅ 5.11 Deploy backend → Render
✅ 5.12 Deploy frontend → Vercel
✅ 5.13 Deploy database → Supabase
```
